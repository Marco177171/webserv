...
void Server::defaultAnswer(int err, t_connection conn)
{
	...
	if (conn.config.error_pages.size() && err != 500)
	{
		...
		for (std::vector<std::string>::iterator it = conn.config.error_pages.begin(); it != conn.config.error_pages.end(); it++)
		{
			if (body.str() == (*it).substr(0, 3))
			{
				...
				if (!file)
				{
					std::cout << "Error: Error page not found" << std::endl;
					file.close();
	...
	while (iter != conn.response.headers.end())
	{
		if ((*iter).first == "Set-Cookie")
		{
			...
			{
				tmpString += (*iter).first + ": " + tmpVal.substr(0, tmpVal.find(";")) + "\r\n";
				tmpVal = tmpVal.substr(tmpVal.find(";") + 1);
			}
			tmpString += (*iter).first + ": " + tmpVal + "\r\n";
		}
	...
	if (err != 100 && conn.request.method != "HEAD")
		...
		tmpString += "\r\n";
	send(conn.socket, tmpString.c_str(), tmpString.size(), 0);
}
...
void	Server::correctPath(std::string &filename, t_connection &conn) {
	std::cout << "-> in Server::correctPath()" << std::endl;
	std::string	tmp;

	(void)conn;
	for (std::vector<t_location>::iterator iter = _config->locations.begin(); iter != _config->locations.end(); ++iter) {
		std::cout << "-> in Server::correctPath() -> _config-locations CYCLE" << std::endl;
		if (filename.compare(0, iter->path.length(), iter->path) == 0 && iter->path.length() > tmp.length()) {
			tmp = iter->path;
		}
	}
	if (!tmp.empty()) {
		std::cout << "-> in Server::correctPath() -> tmp.empty()" << std::endl;
	...
	if (!conn.request.arguments.empty()) {
		std::cout << "-> in Server::correctPath() -> !conn.request.arguments.empty()" << std::endl;
		std::string argKey;
		std::string argValue;
		...
		while (1)
		{
			...
			if (equalSignPos != std::string::npos && conn.request.arguments[equalSignPos - 1] != '=' && findKey(conn.request.headers, "Cookie")->second.find("name") == std::string::npos)
			{
				argKey = conn.request.arguments.substr(0, equalSignPos);
				argValue = conn.request.arguments.substr(equalSignPos + 1, ampersandPos - argKey.length() - 1);
				conn.cookie.insert(std::make_pair(argKey, argValue));
				conn.set_cookie = true;
			}
			if (ampersandPos != std::string::npos)
				conn.request.arguments = conn.request.arguments.substr(ampersandPos + 1);
			else
				break;
		}
	}
...
void	Server::answer(std::string fullPath, struct dirent *dirent, t_connection conn) {
	std::cout << "-> in Server::answer()" << std::endl;
	...
	tmpBody = stream.str();
	stream.str("");
	std::cout << "-> in Server::answer() -> cookiesManagement" << std::endl;
	...
	{
		std::cout << "-> in Server::answer() -> cookiesManagement -> first case" << std::endl;
		std::string value;
		std::string tmpBody2;
	...
	{
		std::cout << "-> in Server::answer() -> cookiesManagement -> second case" << std::endl;
		std::string tmpBody2;
	...
	{
		std::cout << "-> in Server::answer() -> cookiesManagement -> third case" << std::endl;
		std::string tmpBody2;
	...
	if (tmpBody.length())
	{
		std::cout << "-> in Server::answer() -> Body building" << std::endl;
		stream << tmpBody.length();
		stream >> tmpString;
		(*iter2).second = tmpString;
		std::cout << "-> in Server::answer() -> Body building DONE" << std::endl;
	}
	else
		(*iter2).second = "0";
	...
	if (conn.set_cookie)
	{
		std::cout << "-> in Server::answer() -> cookiesManagement -> conn.set_cookie is true" << std::endl;
		std::string tmp;
	...
	std::map<std::string, std::string>::iterator iter = conn.response.headers.begin();
	std::string tmpVal;
	std::cout << "-> in Server::answer() -> cycling headers..." << std::endl;
	while (iter != conn.response.headers.end())
	{
	...
	std::cout << "-> in Server::answer() -> tmpString built with headers" << std::endl;
	responseStream << tmpString;
	...
	{
		usleep(200);
		send(conn.socket, responseString.c_str(), IOV_MAX, MSG_NOSIGNAL);
		responseString = responseString.substr(IOV_MAX);
	}
	usleep(200);
	send(conn.socket, responseString.c_str(), responseString.size(), MSG_NOSIGNAL);
	resource.close();
}
...
void Server::answerAutoIndex(std::string fullPath, DIR *dir, t_connection conn)
{
	...
	std::cout << "WE're in AUTOINDEX function" << std::endl;
	if ((*fullPath.end() - 1) != '/')
		fullPath.push_back('/');
...
void Server::dirAnswer(std::string fullPath, struct dirent* dirent, t_connection conn) {
	std::cout << "-> in Server::dirAnswer()" << std::endl;
	if (*fullPath.rbegin() != '/')
		fullPath.append("/");
	std::string path = dirent ? fullPath + dirent->d_name + "/" : fullPath;
	DIR* dir = opendir(path.c_str());
	std::cout << "-> in Server::firAnswer() -> path : " << path << std::endl;
	if (!dir)
	{
		defaultAnswer(404, conn);
		return;
	...
	{
		std::cout << "-> in Server::correctPath() -> autoindex off or empty" << std::endl;
...
int Server::tryFiles(t_connection &conn) {
	std::cout << "-> in Server::tryFiles()" << std::endl;
	...
	for (; it != conn.config.try_files.end(); it++)
	{
		...
		if ((*it).length())
		{
			...
			std::cout << "tryFiles() -> conn.config.autoindex = true..." << std::endl;
...
int Server::encoding(t_connection &conn)
{
	...
	if (conn.buff_size == -1)
	{
		...
		dataRead = recv(conn.socket, &buffer, sizeof(unsigned char), 0);
		if (dataRead <= 0)
		{
			perror("recv");
			return (1);
	...
	{
		char chunk[4];
		recv(conn.socket, chunk, sizeof(chunk), MSG_DONTWAIT);
		conn.buff_size = -1;
		return (1);
	}
	else
	{
		...
		{
			dataRead = recv(conn.socket, buffer.data() + totalRead, conn.buff_size - totalRead, MSG_DONTWAIT);
			if (dataRead < 0)
			{
				usleep(1000);
			}
		...
		dataRead = recv(conn.socket, buffer.data(), 2, 0);
		conn.buff_size = -1;
		return (0);
	}
}
...
bool Server::contentType(t_connection &conn)
{
	...
	int dataRead = 0;
	dataRead = recv(conn.socket, read, sizeof read, MSG_DONTWAIT);
	...
	if (findKey(connection.request.headers, "Transfer-Encoding")->second == "chunked")
	{
		if (this->encoding(connection) == 1)
		{
			...
			while (1)
			{
				if (size <= 0x8000)
				{
					send(connection.socket, answer, size, 0);
					free(tmp);
					while (recv(connection.socket, NULL, 0, 0) < 0)
						usleep(50);
					return 1;
				}
				send(connection.socket, answer, 0x8000, 0);
				answer += 0x8000;
				size -= 0x8000;
			}
		}
...
int Server::get(t_connection &conn) {
	std::cout << "-> in Server::get()" << std::endl;
	std::vector<std::pair<std::string, std::string> >::iterator it;
	for (it = _cacheRequests.begin(); it != _cacheRequests.end(); it++) {
		if (it->first == conn.headers) {
			conn.request.method.clear();
...
int Server::handleClient(int socket)
{
	...
	{
		usleep(200);
		setsockopt(it->socket, SOL_SOCKET, SO_LINGER, &l, sizeof(l));
		close(it->socket);
		_connections.erase(it);
		return 1;
	}
