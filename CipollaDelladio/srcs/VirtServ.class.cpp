...
bool		VirtServ::saveFiles(std::string value, t_config & ret, t_connInfo & conn)
{
	...
	for (std::vector<std::string>::iterator iter = ret.files.begin(); iter != ret.files.end(); iter++)
	{
...
bool		VirtServ::tryGetResource(std::string filename, t_connInfo conn)
{
	...
	if (!std::strncmp(conn.location->location.c_str(), filename.c_str(), std::strlen(conn.location->location.c_str())))
	{
		...
		std::cout << "CD 1: " << filename << std::endl;
		if (*filename.begin() == '/' && filename.length() > 1)
			...
			filename = "/";
		std::cout << "CD 2: " << filename << std::endl;
		if (filename.find('/') != std::string::npos)
		{
			std::cout << "CD 3: " << filename << std::endl;
			if (*rootPath2.rbegin() != '/')
	...
	std::cout << "CD 4: " << filename << std::endl;
	correctPath(filename, conn);
	std::cout << "final path: " << filename << std::endl;
...
void		VirtServ::answer(std::string fullPath, struct dirent *dirent, t_connInfo conn)
{
	...
	tmpBody = stream.str();
	stream.str("");
	std::cout << "--- NAME : " << findKey(conn.request.headers, "Cookie")->second << findKey(conn.request.headers, "Cookie")->second.find("name") << std::endl;
	...
	{
		send(conn.fd, responseString.c_str(), IOV_MAX, 0); // CRASH
